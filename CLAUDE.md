...
個人開発セクションの端末モックに入れる画像は `public/apps/*.webp`（620x1344）。
`profile.json` の `screenshot` から参照する。**参照切れは `test/content/screenshots.test.ts` が落として教える**
（画像を消しても他のテストとビルドは通ってしまい、サイトで端末モックが空になるだけなので）。

いまの画像は App Store の
[iTunes Lookup API](https://itunes.apple.com/lookup?id=6759832862&country=jp) の `screenshotUrls` から取り、
**端末フレームとキャプションを取り除いて画面部分だけを切り出したもの**。ストアのスクショはマーケ用に
端末の絵が描き込まれているので、そのまま自前のモックに入れると枠が二重になる。

**切り出しに使ったスクリプトと座標はリポジトリに残っていない**（2026-09 時点）。作り直すときは、
ストアの画像ではなく**各アプリのリポジトリにある素のスクリーンショット**（fastlane / launchpad が吐くもの、
`ios-screenshot` スキル参照）を使えば切り出し自体が不要になる。そちらに寄せるのが本筋。

ストアの画像から作り直す場合は、背景色や影が画像ごとに違って自動判定が当てにならないので、
切り出し範囲は目視で測った固定値を使うこと。素のスクショで配信しているアプリは切り出さずそのまま使える。
